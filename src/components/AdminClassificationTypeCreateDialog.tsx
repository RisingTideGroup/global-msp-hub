...
import { useCreateClassificationType } from "@/hooks/useClassificationTypes";
...
interface AdminClassificationTypeCreateDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export const AdminClassificationTypeCreateDialog = ({ open, onOpenChange }: AdminClassificationTypeCreateDialogProps) => {
  ...
  const { mutate: createType, isPending } = useCreateClassificationType();
  ...
  const handleUseCaseChange = (useCaseId: string, checked: boolean) => {
    if (checked) {
      setFormData(prev => ({
        ...
        display_order: { ...prev.display_order, [useCaseId]: 0 }
      }));
    } else {
      setFormData(prev => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Add New Classification Type</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Enter classification type name (e.g., Industry, Skill, etc.)"
              maxLength={50}
              required
            />
          ...
          <div className="flex justify-end space-x-2 pt-4">
            ...
            >
              {isPending ? 'Adding...' : 'Add Classification Type'}
            </Button>
          </div>
        </form>
      </DialogContent>
